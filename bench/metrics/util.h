...
auto make_bool_ops(BSharedVector<T>& a, BSharedVector<T>& b) {
    ...
    return std::vector<std::pair<std::string, std::function<void()>>>{
            {"AND", [&] { a & b; }},
            {"AND CONST", [&] { a & static_cast<T>(9); }},
            {"OR", [&] { a | b; }},
            // {"OR CONST", [&] { a | static_cast<T>(9); }},
            {"XOR", [&] { a ^ b; }},
            {"XOR CONST", [&] { a ^ static_cast<T>(9); }},
            {"NEG", [&] { !a; }},
            {"COMPLEMENT", [&] { ~a; }},
            {"EQ", [&] { a == b; }},
            // {"EQ-CONST", [&] { a == static_cast<T>(9); }},
            {"NEQ", [&] { a != b; }},
            {"GR", [&] { a > b; }},
            // {"GR-CONST", [&] { a > static_cast<T>(9); }},
            {"GE", [&] { a >= b; }},
            {"COMPARE", [&] { a > b; }},
            {"ADD", [&] { a + b; }},
            // {"ADD-CONST", [&] { a + static_cast<T>(9); }},
            {"SUB", [&] { a - b; }},
            {"DIV", [&] { a / b; }},
            {"CONV", [&] { a.b2a(); }},
        };
...
auto make_arith_ops(ASharedVector<T>& a, ASharedVector<T>& b) {
    ...
    return std::vector<std::pair<std::string, std::function<void()>>>{
            ...
            {"SUB", [&] { a - b; }},
            {"MULT", [&] { a* b; }},
            {"MULT-CONST", [&] { a * static_cast<T>(9); }},
            {"DIV", [&] { a / b; }},
            {"DIV-CONST", [&] { a / static_cast<T>(9); }},
